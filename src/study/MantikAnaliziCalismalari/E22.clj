    ...
    (keep
      ...
      (clojure.walk/postwalk
        (fn
          [form]
          (let
            [form (if (vector? form) (vec (keep identity form)) form)]
            (cond
              (and (vector? form) (every? vector? form))
              form
              (and (vector? form) (not (every? string? form)))
              nil
              (and
                (vector? form)
                (every? string? form)
                (some (fn [str-vec]
                        (clojure.string/includes? (clojure.string/lower-case str-vec)
                                                  search-str))
                      form)
                )
              form
