...
(e/defn SliderApp []
        (e/client
          (let [!state (atom {:in          ""
                ...
                v (get (e/watch !state) :v)]
            (dom/div (dom/props {:style {:display     :grid
                                         :width       "40em"
                                         :grid-gap    "0.5em"
                                         :align-items :center}})
                     (dom/h1 (dom/text "Slider  Example")
                             (dom/props {:style {:grid-row    1
                                                 :align-items :center
                                                 }}))

                     (ui4/range v
                                (e/fn [newv] ((swap! !state assoc :v newv)))
                                (dom/props {:min   0,
                                            :max   100,
                                            :style {:grid-row 2}}))
                     ...
                     (dom/button (dom/on "click" (e/fn [click] (swap! !state assoc
                                                                      :v ""
                                                                      :in "")
                                                       ))
                                 (dom/text "Reset!!!")
